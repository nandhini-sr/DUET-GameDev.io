...
function restart()
{
  ...
  rank = rowCnt-1;
  localStorage.setItem('rh'+rank,player +' '+ index);
...
function addRow() {
        ...
        for (var c = 0; c < arrHead.length; c++) {
            ...
            if (c == 0) {  
                ...
                {
                  player = 'A';
                  index = Math.floor((rowCnt/2)) + 1;
                }

                element.innerHTML = player +' '+ index;
        ...
        localStorage.setItem('rh'+ (scoreTab.rows.length - 1), player +' '+ index);
...
function gameloophack(){
    ...
    c.fillStyle = 'black';
	  c.textAlign = 'center';
	  c.fillText('Score: ' + score,350,150);
    c.fillText('Player ' + player,350,100);
...
gameloophack();
   ...
   function nextPlayerGame()
   {
    ...
    colorList = ['#abb91b', '#7f0d92', '#ef8f01'];
    score = 0;
    rank = 0;
    //start_time = 0;
    setMode = 0;
    spre = 0; //for obstacleRelease
    miniDuets = [];
    remaining = []
    num = 0;
